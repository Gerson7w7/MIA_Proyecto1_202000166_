...
string InodoTabla(int indice, INODOS *inode, int tree)
{
    ...
    tableinode += to_string(inode->i_s);
    tableinode += "</td></tr>\n";
...
void repTree(char _id[], char _name[], char _path[], char _dir[])
{
    ...
    char pathdisk[100] = "";
    bool disK_or = existeArchivo(disco.path);
    char pathnew[100] = "";
    if (disK_or)
    {
...
void repJournaling(char _id[], char _name[], char _path[], char _dir[])
{
    ...
    if (!existedot)
    {
        ...
        fs << "<tr><td>Journal_tipo_operacion</td><td>Creacion</td></tr>" << endl; // charToStringR(jour.jorunal_tipo_operacion)
...
void repSuper(char _id[], char _name[], char _path[], char _dir[])
{
    ...
    if (!existedot)
    {
        ...
        fs << "<tr><td>s_inodes_size</td><td> " + to_string(auxsuper.s_inode_s) + " </td></tr>" << endl;
        fs << "<tr><td>s_block_size</td><td> " + to_string(auxsuper.s_block_s) + " </td></tr>" << endl;
...
void repBitmapInode(char _id[], char _name[], char _path[], char _dir[])
{
    ...
    if (!existedot)
    {
        ...
        cout << "lei el superbloque" << endl;
        // se arma la estructura
        // char numeroinodos = auxsuper.s_inode_count;
        char bitinodos[auxsuper.s_inode_count];
        fseek(file, auxsuper.s_bm_inode_start, SEEK_SET);
        ...
        for (int i = 0; i < auxsuper.s_inode_count; ++i)
        { // se cambia numero de inodos
...
void repBitmapBlock(char _id[], char _name[], char _path[], char _dir[])
{
    ...
    if (!existedot)
    {
        ...
        fseek(file, mbr.mbr_particion[indice].part_start + sizeof(mbr.mbr_particion[indice]) + 1, SEEK_SET);
        fread(&auxsuper, sizeof(SUPER_BLOQUE), 1, file);
        char bitbloque[auxsuper.s_block_count];
        fseek(file, auxsuper.s_bm_block_start, SEEK_SET);
        ...
        bool flag_salto = true;
        for (int i = 0; i < auxsuper.s_block_count; ++i)
        {
...
void analisisRep(char comando[])
{
    ...
    while (comando[contador] != NULL)
    {
        ...
        {
            cout << "sistema~> analizando reportes... " << endl;
            memset(lineacomando, 0, 100);
            contador++;
        }
        else if (strcmp(lineacomando, ">name=") == 0)
        {
            ...
            cout << "sistema~> se creara el siguiente reporte: " << _name << endl;
        }
        else if (strcmp(lineacomando, ">id=") == 0)
        {
            ...
            while (comando[contador] != NULL)
            {
                ...
                {
                    char aux[1] = "";
                    aux[0] = comando[contador]; // quito el tolower
                    strncat(_id, aux, 1);
                    contador++;
                }
            }
            cout << "sistema~> id de la particion a reportar: " << _id << endl;
        }
        else if (strcmp(lineacomando, ">path=") == 0)
        {
            ...
            cout << "sistema~> ruta ingresada: " << _path << endl;
        }
    }
    ...
    // empieza a generar solo el directorio con el valor anterior de cont_barra1
    int cont_barra2 = 0;
    int cont_2 = 0;
    while (_path[cont_2] != NULL)
    {
        if (_path[cont_2] == '/')
        {
            char aux[1] = "";
    ...
    cout << "sistema~> Nombre del reporte: " << nombre_rep << endl;
    cout << "sistema~> Nombre del directorio: " << direc << endl;

    // obtengo el nombre del reporte
    bool flag_directorio = validacionDirectorioReporte(direc);
    bool flag_rep = validacionReporte(_path);

    // tipo de reporte de generar

    bool flag_generador = true;
    if (strcmp(_name, "mbr") == 0)
    {
        flag_mbr = true;
        cout << "sistema~> Tipo de reporte si existe: " << _name << endl;
    }
    else if (strcmp(_name, "disk") == 0)
    {
        flag_disk = true;
        cout << "sistema~> Tipo de reporte si existe: " << _name << endl;
    }
    else if (strcmp(_name, "block") == 0)
    {
        flag_block = true;
        cout << "sistema~> Tipo de reporte si existe: " << _name << endl;
    }
    else if (strcmp(_name, "bm_block") == 0)
    {
        flag_bm_block = true;
        cout << "sistema~> Tipo de reporte si existe: " << _name << endl;
    }
    else if (strcmp(_name, "bm_inode") == 0)
    {
        flag_bm_inode = true;
        cout << "sistema~> Tipo de reporte si existe: " << _name << endl;
    }
    else if (strcmp(_name, "inode") == 0)
    {
        flag_inode = true;
        cout << "sistema~> Tipo de reporte si existe: " << _name << endl;
    }
    else if (strcmp(_name, "journaling") == 0)
    {
        flag_journaling = true;
        cout << "sistema~> Tipo de reporte si existe: " << _name << endl;
    }
    else if (strcmp(_name, "tree") == 0)
    {
        flag_tree = true;
        cout << "sistema~> Tipo de reporte si existe: " << _name << endl;
    }
    else if (strcmp(_name, "sb") == 0)
    {
        flag_sb = true;
        cout << "sistema~> Tipo de reporte si existe: " << _name << endl;
    }
    else if (strcmp(_name, "file") == 0)
    {
        flag_file = true;
        cout << "sistema~> Tipo de reporte si existe: " << _name << endl;
    }
    else
    {
        ...
        cout << "Error~> El reporte solicitado es incorrecto " << _name << endl;
    }
    ...
    if (flag_generador)
    {
        ...
        { // existe directorio
            ...
            { // no existe rep
                cout << "sistema~> generando reporte...: " << _name << endl;
                if (flag_mbr)
                {
                    repMbr(_id, nombre_rep, _path, direc);
                }
                else if (flag_disk)
                {
                    repDisk(_id, nombre_rep, _path, direc);
                }
                else if (flag_tree)
                {
                    // se genera rep del arbol
                    repTree(_id, nombre_rep, _path, direc);
                }
                else if (flag_inode)
                { // reporte de inodos en uso
                    repInodos(_id, nombre_rep, _path, direc);
                }
                else if (flag_block)
                { // para el reporte de bloques
                    repBlock(_id, nombre_rep, _path, direc);
                }
                else if (flag_journaling)
                { // reporta para genenara journaling
                    repJournaling(_id, nombre_rep, _path, direc);
                }
                else if (flag_sb)
                { // para superbloque
                    repSuper(_id, nombre_rep, _path, direc);
                }
                else if (flag_bm_inode)
                { // bitmap para inodo
                    repBitmapInode(_id, nombre_rep, _path, direc);
                }
                else if (flag_bm_block)
                { // bitmap para bloques
                    repBitmapBlock(_id, nombre_rep, _path, direc);
                }
            }
        }
        else
        { // no existe directorio
            // se crea directorio
            cout << "sistema~> creando folders necesarios para guardar reporte..." << endl;
            string path = charToString(direc);
            string dir = "mkdir -p \"" + path + "\"";
            system(dir.c_str());
            cout << "Aviso -> Se genera reporte " << _name << endl;
