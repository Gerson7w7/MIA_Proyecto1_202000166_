...
// funcion para crear un folder (directorio)
int createFolder(FILE *file, SUPER_BLOQUE *super, int *indicepadre, string namecarpeta)
{
    ...
    int indiceaotorgarinodo = returnInodeAvailable(super, file);
...
int buscarindirectoSimple(INODOS *inode, int indicedelinodo, FILE *file, SUPER_BLOQUE *super, string namecarpeta, int *indiceactualdeinodo)
{
    ...
    if (indicedelinodo > 0)
    {
        ...
        for (int i = 0; i < 16; ++i)
        {
            ...
            {
                hay_espacio_simple = true;
            }
        }

        if (hay_espacio_simple)
        {
            for (int i = 0; i < 16; ++i)
            {
    ...
    else
    {
        ...
        if (valorapuntador > 0)
        {
            ...
            int valorBloque = returnBlockAvailable(super, file);
            apuntadorsimple.b_apuntadores[0] = valorBloque;
            ...
            bitbloque = '1';
            fseek(file, super->s_bm_block_start + valorBloque * sizeof(char), SEEK_SET);
            fwrite(&bitbloque, sizeof(char), 1, file);
            BLOQUECARPETA carpetadirectorio;
            CONTENT contentpadre;
            strcpy(contentpadre.b_name, "-");
            ...
            carpetadirectorio.b_content[0] = contentpadre;
            fseek(file, super->s_block_start + valorBloque * 64, SEEK_SET);
            fwrite(&carpetadirectorio, 64, 1, file);
            super->s_free_blocks_count--;
...
int buscarEspacio(INODOS *inode, FILE *file, SUPER_BLOQUE *super, int *indiceactualinodo, string namecarpeta)
{
    ...
    bool hay_espacios_indirectos = false; // cambio a true
...
void ejecutarMkdir(DISCO disco, char _id[], char _directorio[], char _path[])
{
    ...
    { // si existe
        ...
        if (auxsuper.s_magic != 0)
        {
            ...
            auxjournaling.journal_persimo = 664;
            ...
            {                                 // hay varias carpetas
                ...
                cout << "sistema~> se ha creado el folder en el directorio indicado correctamente." << endl;
            }
        }
        else
        {
            cout << "Error~> La particion no se ha formateado" << endl;
            fclose(file);
        }
    }
    else
    {
        cout << "Error~> No se ha encontrado la particion montada." << endl;
    }
    ListaCarpetas.clear();
}
...
void analisisMkdir(char comando[])
{
    ...
    while (comando[contador] != NULL)
    {
        ...
        {
            cout << "sistema~> analizando particion para crear una carpeta... "  << endl;
            memset(lineacomando, 0, 200);
            contador++;
        }
        else if (strcmp(lineacomando, ">path=") == 0)
        {
            ...
            cout << "sistema~> el folder se creará en el siguiente directorio: " << _path << endl;
        }
        else if (strcmp(lineacomando, ">r") == 0)
        {
            cout << "sistema~> se crearan los folders necesarios para guardar la carpeta." << endl;
            memset(lineacomando, 0, 200);
            gotR = true;
        }
    }
    ...
    if (!sesionIniciada)
    {
        cout << "Error~> para ejecutar este comando debe de realizar inicio de sesion." << endl;
        return;
    }
    cout << "sistema~> creando carpeta..." << endl;
    if (gotR)
    {
        R = true;
        string idAux = getId();
        cout << "ID de la particion montada es: " << idAux << endl;
        strcpy(_id, idAux.c_str()); // aqui el ID  que tenga el login lo cambio a valor ID
        DISCO disco = buscarDisco(_id, 3);
        ejecutarMkdir(disco, _id, _path, disco.path);
    }
    else
    {
        string idAux = getId();
